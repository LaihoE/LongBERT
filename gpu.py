...
def ask_question(longtext_file_path,question,early_stop_conf,words_per_split=200):
    ...
    for inx in range(len(list_of_batches)):
        ...
        Y=torch.tensor([segment_ids])

        X=X.to(device=device)
        Y=Y.to(device=device)
        model = model.to(device)
        outputs = model(X, # The tokens representing our input text.
                        token_type_ids=Y, # The segment IDs to differentiate question from answer_text
                        return_dict=True)
...
question = 'what was raskolnikovs job?'
